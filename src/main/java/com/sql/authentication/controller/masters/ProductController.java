...
public class ProductController {
    ...
    public ResponseEntity<?> get(){
        try {
            ...
            Response<List<Product>> response=new Response<>(HttpStatus.CREATED.value(),"Success",products);
            return ResponseEntity.ok().body(response);
        }catch (Exception e){
            ErrorResponse response = new ErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR.value(), e.getMessage());
            return ResponseEntity.internalServerError().body(response);
    ...
    public ResponseEntity<?> edit(@RequestParam  @NotNull Integer id){
        try {
            ...
            Response<Product> response=new Response<>(HttpStatus.CREATED.value(),"Success",products);
            return ResponseEntity.ok().body(response);
